#ifndef TESTPROJECT_AXE_SMITH_H
#define TESTPROJECT_AXE_SMITH_H
...
{
public:
    AxeSmith(const glm::vec2& pos = glm::vec2());
...
AxeSmith::AxeSmith(const glm::vec2& pos)
    : Worker(pos, "Smith")
{
...
void AxeSmith::update(float dt)
{
    // Do planning updates
    GoapEntity::update(dt);
}

goap::StateCollection AxeSmith::getGoals() const
{
    goap::StateCollection goals;

    goals[STATE_AXE_AVAILABLE] = true;

    return goals;
}

goap::StateCollection AxeSmith::getWorldState() const
{
